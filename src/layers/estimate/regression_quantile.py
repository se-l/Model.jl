    ...
    def train(self):
        ...
        for quantile in self.quantiles:
            ...
            estimator_params = {
                ...
                'learning_rate': 0.05,
                'boosting_type': 'gbdt',
                'num_iterations': 2, # math.floor(sum([b.best_iteration for b in self.boosters[quantile]]) / len(self.boosters[quantile])),
                'device': 'gpu'
            }
            dataset_train = lgb.Dataset(self.df, label=self.ps_label, weight=arr_weight)
            lgb_booster = lgb.train(estimator_params,
                                    train_set=dataset_train,
                                    valid_sets=[dataset_train],
                                    valid_names=['valid_train'],
                                    )
            logger.info(lgb_booster.best_score)
            yhat = lgb_booster.predict(self.df)
            logger.info(f'MSE predicted: {mean_squared_error(self.ps_label, yhat)}')
            ...
            logger.info(f'''Mean train: {sum([b.best_score['valid_train']['l2'] for b in self.boosters[quantile]]) / 5}''')
            logger.info(f'''Mean valid: {sum([b.best_score['valid_0']['l2'] for b in self.boosters[quantile]]) / 5}''')
